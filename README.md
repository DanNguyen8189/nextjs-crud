# Meltdown!
Just experienced a meltdown? Log it here!
https://meltdowntown.netlify.app/

## What I used
- Next.js React framework with the pages router; I used this starter here: https://www.netlify.com/integrations/community-built/next-netlify-starter-template/
- Prisma ORM to work with SQL database
- Netlify for deployment
- Mantine React components library
- FormData() for form submission

## Why
- I thought it'd be funny 
- I wanted to learn about working with React and databases. In hindsight, maybe I didn't need to start with Next.js but it worked out okay?